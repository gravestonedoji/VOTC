...
// French localization - Just Edition (compact personality values)
...
function personalityDescription(char) {
  ...
  traits.push(`courage: ${char.boldness}`);
  traits.push(`compassion: ${char.compassion}`);
  traits.push(`énergie: ${char.energy}`);
  traits.push(`avarice: ${char.greed}`);
  traits.push(`honneur: ${char.honor}`);
  traits.push(`rationalité: ${char.rationality}`);
  traits.push(`sociabilité: ${char.sociability}`);
  traits.push(`rancune: ${char.vengefulness}`);
  traits.push(`zèle: ${char.zeal}`);

  return `noyau de personnalité: \n${traits.join("\n")}`;
}
