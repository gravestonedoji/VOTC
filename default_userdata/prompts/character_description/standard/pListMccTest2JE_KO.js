...
// Korean localization - Just Edition (compact personality values)
...
function personalityDescription(char) {
  ...
  traits.push(`용감함: ${char.boldness}`);
  traits.push(`자비로움: ${char.compassion}`);
  traits.push(`근면함: ${char.energy}`);
  traits.push(`탐욕: ${char.greed}`);
  traits.push(`명예: ${char.honor}`);
  traits.push(`이성: ${char.rationality}`);
  traits.push(`사교성: ${char.sociability}`);
  traits.push(`앙심: ${char.vengefulness}`);
  traits.push(`열성: ${char.zeal}`);
