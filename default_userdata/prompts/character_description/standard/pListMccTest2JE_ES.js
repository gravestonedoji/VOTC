...
// Spanish localization - Just Edition (compact personality values)
...
function personalityDescription(char) {
  ...
  traits.push(`valentía: ${char.boldness}`);
  traits.push(`compasión: ${char.compassion}`);
  traits.push(`energía: ${char.energy}`);
  traits.push(`codicia: ${char.greed}`);
  traits.push(`honor: ${char.honor}`);
  traits.push(`racionalidad: ${char.rationality}`);
  traits.push(`sociabilidad: ${char.sociability}`);
  traits.push(`venganza: ${char.vengefulness}`);
  traits.push(`celo: ${char.zeal}`);

  return `núcleo de personalidad: \n${traits.join("\n")}`;
}
